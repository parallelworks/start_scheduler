...
if ! [ -f "${GT_CONF}" ]; then
    ...
    sed -i "s|.*GTDistributed.executor.core-count.*|GTDistributed.executor.core-count = ${cpe}|g" ${GT_CONF}
    sed -i "s|.*GTDistributed.executor.priority.*|GTDistributed.executor.priority = ${priority}|g" ${GT_CONF}
fi
