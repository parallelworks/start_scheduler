...
def kill_job(job_name):
    # FIXME: Check if job has already started?
...
def get_product_alerts(alerts_json, limits):
    # If alerts_json does not have alerts for all products!
    update_alerts_json = False
    try:
    ...
    for product in limits.keys():
        if product not in alerts:
            print("WARNING: Product {} not found in alerts file {}!".format(product, alerts_json))
            print("WARNING: Product {} added to file {} automatically!".format(product, alerts_json))
            update_alerts_json = True
            alerts[product] = {"warning": False, "hardstop": False}

    if update_alerts_json:
