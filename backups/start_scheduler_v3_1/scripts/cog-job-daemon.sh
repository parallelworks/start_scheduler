...
exec_log_dir=$7
# Optional:
cpe=$8 # Cores per executor
sched_licenses=$9 # Licenses per scheduler
...
if ! [ -f pool_info.json ]; then
   echo "ERROR: File pool_info.json was not produced by command:"
   echo "ERROR: curl https://beta.parallel.works/api/resources?key=${api_key}"
   exit 0
fi
...
if [[ ${controlport} == "None" ]]; then
    echo "ERROR: No control port found for pool ${pool_name}"
    echo "ERROR: Make sure pool ${pool_name} is on!"
    exit 0
fi
...
# Executor commands:
stream_out_cmd="bash stream.sh beta.parallel.works ${exec_log_dir}/\${HOSTNAME}.out ${exec_work_dir}/\${HOSTNAME}.out"
stream_err_cmd="bash stream.sh beta.parallel.works ${exec_log_dir}/\${HOSTNAME}.err ${exec_work_dir}/\${HOSTNAME}.err"
wti_cmd="bash ${exec_work_dir}/wti.sh ${GT_USER} ${sched_ip} 1>${exec_work_dir}/\${HOSTNAME}.out 2>${exec_work_dir}/\${HOSTNAME}.err"
executor_cmds="${stream_out_cmd}; ${stream_err_cmd}; ${wti_cmd}"
echo Executor commands:
echo ${executor_cmds} | tr ";" "\n"
...
# If the scheduler licenses are not defined (<1) get the maximum licenses
# This is to prevent starting cores with no free licenses to run
if [ ${sched_licenses} -lt 1 ]; then
    license_info=$(perl ${apps_dir}/lmstatparse.pl -f GTsuite)
    sched_licenses=$(echo ${license_info} | awk '{print $3}')
fi
echo "Scheduler licenses: ${sched_licenses}"

# Global completed packets counter:
count_core_demand(){
    secure_curl "curl http://${sched_ip}:8979/jobs/?xml" webapp.xml
    python3 ${apps_dir}/count_core_demand.py webapp.xml ${sched_licenses}
}
...
cjs() {
    ...
    wait_till_iddle_map="${apps_dir}/wti.sh -> ${exec_work_dir}/wti.sh : stream.sh -> ${exec_work_dir}/stream.sh"
...
while true; do
    ...
    date
    secure_curl "curl ${CONTROLURL}/getAll" ${pool_name}.json
    max_pool_slider=$(cat ${pool_name}.json | tr ',' '\n' | grep max | cut -d':' -f2)
