...
if __name__ == "__main__":
    ...
    lmstat_out = sys.argv[3]

    pp = PrettyPrinter(depth=4)
    core_demand_txt = sched_work_dir + "/CORE_DEMAND"

    job_records_json = sched_work_dir + "/job_records.json"
    usage_json = sched_work_dir + "/usage.json"
    limits_json = sched_work_dir + "/product_limits.json"
    alerts_json = sched_work_dir + "/alerts.json"

    [job_records, usage, limits, alerts] = json2dict([job_records_json, usage_json, limits_json, alerts_json])

    lic_info = lmstat.get_lic_info(lmstat_out)
    ...
    usage = lmstat.update_usage(usage, lic_info)
    alerts = alerts_mod.update(alerts, usage, limits)
    dict2json([job_records, usage, limits, alerts], [job_records_json, usage_json, limits_json, alerts_json])
