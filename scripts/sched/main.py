...
elif '_' in pool_names:
    pool_names = pool_names.split('_')
else:
...
inputs = ["scripts/wti.sh", gtdist_exec_pfile + " -> " + "/tmp/gtdistd-exec.properties"]
for pname,nworkers in exec_overdemand.items():
    ...
    for i in range(nworkers):
        ...
        stdout = exec_work_dir + "/wti.out"
        stderr = exec_work_dir + "/wti.err"
